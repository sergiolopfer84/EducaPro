...
public class TestService {
	...
	public List<NotaHistorialDTO> obtenerHistorialNotas() {
		return testRepository.findAll().stream()
				.map(test -> new NotaHistorialDTO(test.getNombreTest(), puntuacionRepository.findNotasByTest(test)))
	...
	public Test toggleEstado(int id) {
		Test test = testRepository.findById(id).orElseThrow(() -> new RuntimeException("❌ Test no encontrado"));
